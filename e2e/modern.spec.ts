...
test('display and audio preferences persist; reduced motion removes map traffic', async ({ page }) => {
  ...
  await expect(traffic).toBeHidden()
...
test('maintenance and standby are deliberate fleet decisions', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /^Book check/ }).click()
  await expect(page.getByTestId('operations-panel')).toContainText('Check booked · week 1')
  await expect(page.getByRole('button', { name: /^Reschedule check/ })).toBeEnabled()
  expect(await page.evaluate(() => { const o = window.__harness.getState()!.airlines[0]!.fleet[0]!.operations!; return o.checkEnd! - o.checkStart! })).toBe(7 * 24 * 60)
