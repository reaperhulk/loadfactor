...
for(const [width,height,textSize] of [[1440,900,100],[390,844,100],[768,1024,125],[1366,768,125]] as const) test(`recorded workload ${width}px ${textSize}%: mature planning, globe, memory and save reload`,async({page,browserName},info)=>{
  ...
  await page.waitForFunction(()=>['CapitalOutlook','OperationsBoard','FinancePanel','RivalsPanel'].every(name=>performance.getEntriesByType('resource').some(r=>r.name.includes('/'+name+'-'))))
  await page.context().setOffline(true)
  await openPanel(page,'outlook');await expect(page.getByTestId('capital-outlook')).toContainText('Lowest cash')
  await openPanel(page,'operations');await expect(page.getByTestId('operations-board')).toContainText('Aircraft and cover')
  await openPanel(page,'finance');await expect(page.getByTestId('customer-identity-0')).toBeVisible()
  await openPanel(page,'rivals');await expect(page.getByTestId('rivals-panel')).toBeVisible()
  await page.context().setOffline(false)
  console.log(`WORKLOAD ${JSON.stringify(metrics)}`)
