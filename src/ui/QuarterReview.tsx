...
export function QuarterReview({ state, forecast, onClose, onConfirm }: {
  ...
  return <Dialog label="Review quarter" className="gameover-overlay" testId="quarter-review" onClose={onClose}>
    <div className="report-card quarter-review review-layout">
      ...
      <p className="review-profit"><small>Planned company net profit</small><strong className={forecast.profit >= 0 ? 'pos' : 'neg'}>{money(forecast.profit)}</strong>{range.low !== range.high && <span className="review-range" data-testid="review-range">Likely {money(range.low)} to {money(range.high)}</span>}</p>
