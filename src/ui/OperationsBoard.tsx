...
function WeeklyOperationsBoard({state,active=true}:{state:GameState;active?:boolean}) {
  ...
  // Consecutive weeks that read the same fold into one span: a healthy
  // aircraft is one wide cell ("5 trips/wk × 13"), and only the aircraft in
  // focus (the one selected for an experiment) opens into its 13 cells. At
  // eighty aircraft the fully expanded grid was eleven thousand DOM nodes.
  const spansFor=(weeks:typeof result.calendar)=>{
    const spans:{from:number;to:number;week:typeof weeks[number]}[]=[]
    for(const w of weeks){
      const last=spans[spans.length-1]
      if(last && last.to===w.week-1 && last.week.completed===w.completed && last.week.covered===w.covered && last.week.cancelled===w.cancelled && !last.week.unavailableMinutes && !w.unavailableMinutes) last.to=w.week
      else spans.push({from:w.week,to:w.week,week:w})
    }
    return spans
  }
  const cellLabel=(a:{id:number},from:number,to:number,w:typeof result.calendar[number])=>`Aircraft ${a.id} ${from===to?`week ${from+1}`:`weeks ${from+1}–${to+1}`}: ${w.completed} completed, ${w.covered} covered, ${w.cancelled} cancelled`
  const cellClass=(w:typeof result.calendar[number])=>`${w.unavailableMinutes?'week-check':''} ${w.cancelled?'week-gap':''}`
  return <section className="operations-board" data-testid="operations-board"><div className="page-heading"><div><span className="eyebrow">Quarter calendar</span><h2>Aircraft and cover</h2></div></div><p className="hint">Weeks show preferred flights completed, cover received and known downtime. Select an aircraft to open its weeks, then a week to try a check. Changes join your shared plan after review.</p>
    ...
    <div className="table-scroll calendar-scroll"><table className="fleet-calendar"><thead><tr><th>Aircraft / base</th>{Array.from({length:13},(_,i)=><th key={i}>W{i+1}</th>)}</tr></thead><tbody>{airline.fleet.map(a=>{
      const weeks=byAircraft.get(a.id)??[], open=a.id===aircraftId
      return <tr key={a.id} className={open?'calendar-open':undefined} data-testid={`calendar-row-${a.id}`}><th><button aria-expanded={open} onClick={()=>{setAircraftId(a.id);setPreview(false)}}>#{a.id} {getAircraftType(a.type).name}</button><small>{aircraftBase(airline,a)} · {crewFamily(a.type)}</small></th>{open
        ? weeks.map(w=><td key={w.week}><button className={cellClass(w)} aria-label={cellLabel(a,w.week,w.week,w)} onClick={()=>{setWeek(w.week);setAction('check');setPreview(true)}}><strong>{w.completed}</strong><small>{w.unavailableMinutes ? `${Math.ceil(w.unavailableMinutes/60)}h off` : `${Math.floor(w.flightMinutes/60)}h flying`}</small>{w.covered>0 && <small>{w.covered} cover</small>}{w.cancelled>0 && <small>{w.cancelled} gaps</small>}</button></td>)
        : spansFor(weeks).map(sp=><td key={sp.from} colSpan={sp.to-sp.from+1}><button className={`week-span ${cellClass(sp.week)}`} aria-label={cellLabel(a,sp.from,sp.to,sp.week)} onClick={()=>{setAircraftId(a.id);setWeek(sp.from);setAction('check');setPreview(true)}}><strong>{sp.week.completed}</strong>{sp.to>sp.from && <small>× {sp.to-sp.from+1} wks</small>}{sp.week.cancelled>0 && <small>{sp.week.cancelled} gaps</small>}</button></td>)}</tr>
    })}</tbody></table></div>
