...
export function OperationsPanel({ state, selectedAircraftId, mode = 'all' }: { state: GameState; selectedAircraftId?: number; mode?: 'all' | 'policy' | 'aircraft' }) {
  const seat = viewSeat()
  ...
  const { ac, choices, selected, quote } = useMemo(() => {
    ...
    const quote = mode !== 'policy' && ac && selected ? forecastReplacement(state, seat, ac.id, selected.id, leased, finance) : null
  ...
  return <section className="operations-panel" data-testid={mode === 'policy' ? 'fleet-policy' : 'operations-panel'}>
    {mode !== 'aircraft' && <><h3>Fleet policy</h3>
    ...
    {ac && mode !== 'policy' && <details open={mode === 'aircraft' ? true : undefined}>
      <summary>Maintenance, rotations & replacement</summary>
      {selectedAircraftId === undefined && <label>Aircraft <select aria-label="operations aircraft" value={ac.id} onChange={(e) => setAircraftId(Number(e.target.value))}>{airline.fleet.map((a) => <option key={a.id} value={a.id}>#{a.id} {getAircraftType(a.type).name} · {(a.ageQuarters / 4).toFixed(1)} years</option>)}</select></label>}
