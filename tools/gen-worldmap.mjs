...
// Two levels, mirroring the flat map's WORLD_PATH / WORLD_PATH_FINE: the
// globe's quality should match the projection you happen to prefer.
const round1 = ([lon, lat]) => [Math.round(lon * 10) / 10, Math.round(lat * 10) / 10]
const round2 = ([lon, lat]) => [Math.round(lon * 100) / 100, Math.round(lat * 100) / 100]
function sphereRingsAt(toleranceDeg, minPoints, round) {
  const out = []
  for (const ring of allLandRings) {
    const simplified = simplify(ring, toleranceDeg)
    if (simplified.length >= minPoints) out.push(simplified.map(round))
  }
  return out
}
const sphereRings = sphereRingsAt(0.35, 6, round1)
const sphereRingsFine = sphereRingsAt(0.09, 5, round2)
const ringsJson = JSON.stringify(sphereRings)
const ringsFineJson = JSON.stringify(sphereRingsFine)

// Borders for the globe: the same internal-border mesh as lon/lat polylines.
const borderLines = []
for (const line of linesOf(borderMesh)) {
  const simplified = simplify(line, 0.25)
  if (simplified.length >= 2) borderLines.push(simplified.map(round1))
}
const borderLinesJson = JSON.stringify(borderLines)

// Islet positions for the globe: it projects at render time, so it needs the
// lon/lat, not a baked circle.
const isletPoints = []
for (const c of cities) {
  if (islets.includes(c.id)) isletPoints.push(round1([c.lon, c.lat]))
}
...
// Globe coordinates duplicate the flat paths. Ship them only when the player
// requests the globe, keeping their parsing and allocation off the flat map.
const globeOut = `// GENERATED by tools/gen-worldmap.mjs — do not edit by hand.
// Natural Earth 1:50m (public domain). Loaded on demand by the globe view.
export const WORLD_RINGS: readonly (readonly (readonly [number, number])[])[] = ${ringsJson}
export const WORLD_RINGS_FINE: readonly (readonly (readonly [number, number])[])[] = ${ringsFineJson}
export const BORDER_LINES: readonly (readonly (readonly [number, number])[])[] = ${borderLinesJson}
export const ISLET_POINTS: readonly (readonly [number, number])[] = ${JSON.stringify(isletPoints)}
`
writeFileSync(join(root, 'src/data/globemap.gen.ts'), globeOut)
console.log(
  `land ${Math.round(WORLD_PATH.length / 1024)}KB (fine ${Math.round(WORLD_PATH_FINE.length / 1024)}KB) · ` +
    ...
    `(${Math.round(ringsJson.length / 1024)}KB, fine ${sphereRingsFine.length} rings ` +
    `${Math.round(ringsFineJson.length / 1024)}KB, borders ${Math.round(borderLinesJson.length / 1024)}KB) · ` +
    `viewport ${W}x${H}`,
)
